...
//Rare exceptions to indicate a bug in the _expomobile code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_expomobile = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_expomobile]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.expomobile.module"
