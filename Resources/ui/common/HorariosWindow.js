/*function HorariosWindow(Window, params) 
{
    ...
    //params["titulo"]
    //params["subtitulo"]
    //params["servicio_dias"]
    //params["servicio_dia_detalle"]
	  
	var horariosWdw = Titanium.UI.createWindow({
		tabBarHidden : true,
	...
	horariosWdw.add(imageViewBar);
	horariosWdw.add(scrollView_1);

	var dias;
	var elementosCargados = new Array(); //cada propiedad es un dia que contiene un arreglo de eventos
	var elementosCargadosLabels = new Array(); //cada propiedad es un dia que contiene un arreglo de eventos

	network.getData(params["servicio_dias"],function(response) 
	{
		dias = response;
		populateTable();
	}); 
	...
	function populateTable() 
	{
		...
		dias.forEach(function(dia) 
		{
			...
			elementosCargados[titulo.toString()] = new Array();
	...
	table.addEventListener('click', function(e) 
	{
		if (e.row.isparent) 
		{
			...
			else
			{
				...
				if(elementosCargados[e.rowData.title].length == 0)
				{
					network.getDataParam(params["servicio_dia_detalle"].e.row.id, function(response) 
					{
						...
						elementosCargados[e.rowData.title] = eventos;
						elementosCargadosLabels[e.rowData.title] = labelsEventos;
				...
				else //Si ya estaban cargados los eventos solo insertarlos
				{
					...
					for (var i = 0; i < elementosCargados[e.rowData.title].length; i++) 
					{
						table.insertRowAfter(e.index, elementosCargadosLabels[e.rowData.title][i],{animationStyle:Titanium.UI.iPhone.RowAnimationStyle.RIGHT});
					}
		...
		{
			//Mostrar Detalles
			for(var eventosDelDia in elementosCargados)
		    {
		      elementosCargados[eventosDelDia].forEach(function(parEvento)
		      {
	...
	horariosWdw.addEventListener('android:back', function(e) 
	{
		cerrar();		
	});
